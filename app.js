...
const transporter = nodemailer.createTransport({
    ...
    secure: false,
    auth: {
        user: 'ali.abdulla.nurtas@mail.ru',
        pass: '32EkUy24Nyb2JA7wuBhb'
    },
});

const sendWelcomeEmail = (email, username) => {
    const mailOptions = {
        from: 'ali.abdulla.nurtas@mail.ru', // От кого
        to: email, // Кому
...
const mongoDB = 'mongodb+srv://aliabdullanurtas:Alikosh12@cluster0.foz4noe.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0';
...
const createAdminUser = async () => {
    try {
        const userExists = await User.findOne({ username: 'Ali' });
        if (!userExists) {
            const hashedPassword = await bcrypt.hash('Ali12', 10);
            ...
            console.log('Администратор Ali успешно создан');
        }
    } catch (err) {
        console.error('Ошибка при создании администратора Ali:', err);
    }
};
